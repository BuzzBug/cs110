...
static void quicksort_conservative_helper(vector<int>& numbers, ssize_t start, ssize_t finish)
{
  if (start >= finish) return;
  ssize_t mid = partition(numbers, start, finish);
  thread t = thread(quicksort_conservative_helper, ref(numbers), start, mid - 1); 
  quicksort_conservative_helper(numbers, mid + 1, finish);
  t.join();  
}

static void conservativeQuicksort(vector<int>& numbers) {
  quicksort_conservative_helper(numbers, 0, numbers.size() - 1);
}

static void quicksort_threaded_helper(vector<int>& numbers, ssize_t start, ssize_t finish)
{
  if (start >= finish) return;
  ssize_t mid = partition(numbers, start, finish);
  thread tvec[2];
  tvec[0] = thread(quicksort_threaded_helper, ref(numbers), start, mid - 1);
  tvec[1] = thread(quicksort_threaded_helper, ref(numbers), mid + 1, finish);
  tvec[0].join();
  tvec[1].join();
}

static void aggressiveQuicksort(vector<int>& numbers) {
  quicksort_threaded_helper(numbers, 0, numbers.size() - 1); 
}
