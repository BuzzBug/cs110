...
void HTTPRequest::AddFieldsToHeader(const string& clientIPAddress)
{
  string headerkey("x-forwarded-proto");
  string headervalue("http");
  requestHeader.addHeader(headerkey, headervalue);
  string headerkeychain("x-forwarded-for");
  string headervaluechain;
  if(requestHeader.containsName(headerkeychain))
  {
    headervaluechain = requestHeader.getValueAsString(headerkeychain);
    headervaluechain += (", " + clientIPAddress);
    requestHeader.removeHeader(headerkeychain);
  }
  else
  {
    headervaluechain = clientIPAddress;
  }
  requestHeader.addHeader(headerkeychain, headervaluechain);
}

void HTTPRequest::ingestHeader(istream& instream, const string& clientIPAddress) {
  requestHeader.ingestHeader(instream);
  AddFieldsToHeader(clientIPAddress);  
}
...
ostream& operator<<(ostream& os, const HTTPRequest& rh) {
  string result = ((rh.RequestUsesProxy) ? rh.url: rh.path); 
