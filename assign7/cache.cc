...
HTTPCache::HTTPCache(): mutexarray(997), maxAge(-1) {
...
std::mutex& HTTPCache::GetHashedMutex(const HTTPRequest& request)
{
  size_t index = hashRequest(request);
  size_t index_wrapped = index % 997;
  return mutexarray[index_wrapped];
}
